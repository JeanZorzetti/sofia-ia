...
- **G4 — Viz: nós de tarefa + arestas de dependência. ✅ FEITO (2026-06-17).** Pura visualização (motor INTOCADO): SSE [stream/route.ts](../../../src/app/api/teams/[id]/runs/[runId]/stream/route.ts) passou a carregar `dependsOn` (no `select` das tasks **e** no payload do evento `board`; a assinatura `sig` ficou intacta pois deps são imutáveis). Builder PURO novo [team-graph-view.ts](../../../src/lib/orchestration/team/team-graph-view.ts) (`buildTeamGraph(members, tasks, activeId) → {nodes, edges}`, **sem React/DOM e sem runtime de `@xyflow/react`** — só `import type`, erased) e [TeamGraph.tsx](../../../src/app/dashboard/teams/[id]/TeamGraph.tsx) virou renderer fino. **Decisões:** (A) **board-driven** — nós de tarefa renderizam em TODO run (linear simplesmente sem aresta `task→task`, pois `dependsOn=[]`); **layout MANUAL** (sem dagre/elk/d3) — membros nas linhas fixas (lead→workers→reviewer) e cada tarefa empilhada numa coluna sob o dono (assignee); cor por status reusando a paleta do kanban (doing=blue, review=purple, done=emerald, rejected=red, todo=white/30, **blocked=amber + badge 🔒** = "esperando", distinto do vermelho de rejeitado); **chip 🛡 de reviewer** só enquanto `review`; arestas `owner→task` (ownership) e `task→task` (dependência, `dep→dependent`, animada). [TeamRunView.tsx](../../../src/app/dashboard/teams/[id]/TeamRunView.tsx): `dependsOn` no tipo `BoardTask` + passa `tasks` ao `<TeamGraph>`. Sem dep nova, sem schema/migração, coordinator INTOCADO. *Verif.:* `scripts/g4-verify.ts` (32 asserts via tsx sobre o builder puro: 1 nó/tarefa, owner→task, task→task, 6 cores distintas, badge blocked, chip reviewer, regressão linear=0 arestas dep, dep stale ignorada) + **g1/g2/g3 seguem 16/22/18** + typecheck limpo. **E2E autenticado em prod pendente com o usuário** (time `topology: 'graph'` com deps → ver tarefas migrando `todo→doing→review→done`, setas `task→task` e badge `blocked`).
- **G4.1 — Toggle Linear/Grafo na UI (criar/editar time). ✅ FEITO (2026-06-17).** Fechou a lacuna pós-G4 (o modo grafo só ligava via `Team.config.topology` por `PATCH`/DB; nenhuma fatia G0–G6 previa seletor). **Sem tocar engine/executor/SSE/`TeamGraph`** (G4 já renderiza board-driven) — só o form de criar/editar. Módulo PURO novo [team-config-ui.ts](../../../src/lib/orchestration/team/team-config-ui.ts): `buildTeamConfig(base, form)` (substitui o `mergeRepoConfig` inline; faz merge raso preservando chaves não-relacionadas tipo `maxTurns`/`retryCap`, **omite `topology` no linear** = default limpo, só grava `'graph'` como opt-in explícito; `maxParallel` só persiste como int ≥1 em modo grafo) + `topologyOf`/`maxParallelOf` (read-back ao editar). UI em [page.tsx](../../../src/app/dashboard/teams/page.tsx): sub-componente `GraphTopologyField` (segmented Linear/Grafo, default linear + campo opcional **Máx. paralelo** quando grafo, dica `1` p/ modelos `:free`) no `TeamFormModal` compartilhado (criar+editar); state `topology`/`maxParallel` nos dois fluxos; create monta `config` via `buildTeamConfig({},…)`, edit lê via `topologyOf`/`maxParallelOf` e salva via `buildTeamConfig(editConfig,…)`. `POST`/`PATCH /api/teams` já aceitam `config: z.record(z.string(), z.unknown())` → **sem mudança de schema/validação, sem migração, sem dep nova.** *Verif.:* `scripts/g4_1-verify.ts` (20 asserts via tsx sobre o builder puro: linear omite a chave, grafo grava, maxParallel parse/drop, preserva chaves alheias, repo sem regressão, read-back round-trip) + `npm run typecheck` limpo. **E2E autenticado em prod pendente com o usuário** (criar time com Grafo → `config.topology==='graph'`; editar linear↔grafo pelo modal; run do time grafo dispara `runTeamGraph`).
- **G5 — Partículas de handoff + estado ao vivo. ✅ FEITO (2026-06-17).** Pura viz (motor E SSE INTOCADOS — o evento `message` já carrega `fromMemberId`/`toMemberId`/`kind`). Builder PURO [team-graph-view.ts](../../../src/lib/orchestration/team/team-graph-view.ts) ganhou **4º parâmetro OPCIONAL** `opts?: { handoff?: { fromMemberId, toMemberId } | null; running?: boolean }` (assinatura posicional G4 preservada → `g4-verify` segue 32/32): a aresta member↔member cuja direção casa o handoff (`isHandoff(source,target)` genérico, só enquanto `running`) ganha `animated:true` + `HANDOFF_EDGE` (cyan `#22d3ee`, strokeWidth 3, distinto do glow branco genérico do `activeId`); o membro ativo ganha `data.thinking = true` + className `rf-thinking` quando `running`. [TeamRunView.tsx](../../../src/app/dashboard/teams/[id]/TeamRunView.tsx) deriva o **handoff corrente** do último `message` com `kind ∈ {assignment, review}` (Lead→Worker / Worker→Reviewer), re-derivando a cada mensagem e sumindo no terminal; passa `handoff`/`running` ao `<TeamGraph>`. [TeamGraph.tsx](../../../src/app/dashboard/teams/[id]/TeamGraph.tsx) repassa ao builder e injeta o keyframe `rfThinkingPulse` (pulso do `rf-thinking`). **Decisões (confirmadas):** (B) `edge.animated` do React Flow no v1 (dot via `<animateMotion>` = upgrade de fidelidade futuro); janela = **só o último handoff enquanto `running`**. **Board-driven** (vale p/ linear E graph — handoffs acontecem nos dois). Sem dep nova, sem schema/migração, sem tocar SSE/coordinator/agenda/concurrency. *Verif.:* `scripts/g5-verify.ts` (18 asserts via tsx sobre o builder puro: assignment anima `l-<worker>` e só ela · review anima `<worker>-r` · sem handoff/legacy 3-arg → 0 arestas de handoff · handoff exige `running` · membro ativo `data.thinking===true`+className só com `running`; backward-compat) + **g4/g1/g2/g3/g4_1 seguem 32/16/22/18/20** + typecheck limpo. **E2E autenticado em prod pendente com o usuário** (rodar time — `topology:'graph'` de preferência — e confirmar ao vivo: atribuição anima Lead→Worker, review anima Worker→Reviewer, membro ativo pulsa como "thinking").
- **G6 — Clarificação & escalação (`@CLARIFY`). ✅ FEITO (2026-06-17).** Roteamento dinâmico de dúvidas: um Worker sem informação essencial responde `@CLARIFY <pergunta>` em vez de adivinhar → a tarefa entra em **espera** (`status: 'clarify'`, escalada ao Lead, pergunta vira `message`); o Lead responde `@CLARIFY [#n] <resposta>` no turno de planejamento → a tarefa volta a `todo` com a resposta como `reviewNote` (sem incrementar `retryCount` — não é rejeição) e o Worker **re-executa** vendo a resposta no prompt. **SEM migração** (`TaskStatus` é `String @db.VarChar(20)` → `'clarify'` cabe; resposta reusa o canal `reviewNote`); **SEM dependência nova**; **`runTeam` linear INTACTO**. **Parsers PUROS** ([team-protocol.ts](../../../src/lib/orchestration/team/team-protocol.ts)): `parseWorkerOutput(text)` (NOVO, espelha `parseReviewVerdict`: `@CLARIFY` no início de linha → `{kind:'clarify',question}`, senão `{kind:'result'}`) + `parseLeadActions` estendido (`@CLARIFY [#n] resposta` → `{type:'clarify',display,answer}`, convive com `@TASK`/`@MESSAGE`/`@DONE`). **Tipos aditivos** ([team-types.ts](../../../src/lib/orchestration/team/team-types.ts)): `TaskStatus += 'clarify'`, `TaskAction += 'clarify'`, `LeadAction += {display?,answer?}`. **Agenda** ([team-graph-agenda.ts](../../../src/lib/orchestration/team/team-graph-agenda.ts)): `clarify`→`{nextAction:'clarify',actionOwner:'lead'}` logo após `review` (não-terminal como `blocked` → `isBoardSettled` INTOCADO mantém a run viva). **Coordinator** ([team-graph-coordinator.ts](../../../src/lib/orchestration/team/team-graph-coordinator.ts)): EXECUTE roda `parseWorkerOutput` após o chat (clarify → `status:'clarify'` + message worker→lead, sem `result`, sem review); PLANNING resolve `#n`→id real (reusa `displayToId`) e re-enfileira `todo`+`reviewNote`. **Proteção do linear**: a instrução `@CLARIFY` entra via **param OPT-IN** `buildTaskPrompt(task,feedback,{allowClarify:true})` que SÓ o `runTeamGraph` passa (o linear chama com 2 args → byte-idêntico, nunca instruído a clarificar). **Contrato/snapshot do Lead aditivos** ([team-prompts.ts](../../../src/lib/orchestration/team/team-prompts.ts)): coluna `[CLARIFY]` com a pergunta + diretiva `@CLARIFY [#n]` só aparecem quando há `clarify` no board (byte-idêntico sem dúvida pendente). Coordinator linear ignora ação `clarify` (loop só trata `task`/`message`) → zero regressão. *Verif.:* `scripts/g6-verify.ts` (35 asserts via tsx: parsers + state-machine + `allowClarify` opt-in + contrato/snapshot aditivos + **E2E memory-store** do ciclo clarify→escala→responde→re-executa→done, retry=0, reviewer chamado 1×, run completa) + **g1/g2/g3/g4/g4_1/g5 seguem 16/22/18/32/20/18** + `npm run typecheck` limpo. **🏁 ROADMAP G0–G6 COMPLETO.** **Deferido p/ G6.1 (se um dia):** escalação ao **USUÁRIO** (`needsClarification: user` — exigiria pausar a run + input na UI + resume, novo `RunStatus waiting_for_input` + endpoint) e **handoff cross-team**; viz de cor/badge para `clarify` (OPCIONAL, não feito — a tarefa só não muda de cor no grafo). **E2E autenticado em prod pendente com o usuário** (time `topology:'graph'` com missão ambígua → Worker pede esclarecimento, Lead responde, Worker re-executa, run conclui).
