...
Sprint 4 — Dívida técnica e hardening 🟢 ✅ itens 2-5 DONE + pushed main (2026-06-17); item 1 (drop de órfãos) DIAGNOSTICADO, execução deferida p/ Sessão 6 (decisão do usuário — drop em prod é irreversível).
...
Item 1 (modelos órfãos) — DIAGNÓSTICO FEITO, drop p/ Sessão 6: 7 candidatos verificados um-a-um → 4 órfãos REAIS (Campaign/campaigns [relação User.campaigns; legado→ThreadsCampaign], WhatsappInstance/whatsapp_instances [legado; Evolution API+AgentChannel substituíram], Workspace/workspaces [legado→Organization], ComplianceLog/compliance_logs [UI LGPD existe mas rotas /api/settings/compliance/* NUNCA criadas]) + 3 FALSOS-POSITIVOS eliminados (DocumentEmbedding=raw SQL pgvector, ABTestInteraction=include em ab-tests/[id], ThreadsCampaignPost=include em threads/campaigns). Decisão do usuário: ComplianceLog tratado como dívida morta (dropar junto, não preservar). Drop exige precheck de contagem + olhar dados em prod (lição 6g) + backup JSON + migração + migrate deploy manual no host real ANTES do push.
