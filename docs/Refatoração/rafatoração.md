...
Sprint 3 — Performance: bundle e rendering 🟡 ✅ DONE (2026-06-16) — sem migração (trabalho de client/build).
[STATUS] Entregue:
- next.config: experimental.optimizePackageImports = [recharts, framer-motion, react-syntax-highlighter, lucide-react] (tree-shake de barrel-exports) + alias de polyfill-module → src/lib/empty-polyfills.js, cobrindo turbopack.resolveAlias E webpack (lição nextjs_unconditional_polyfills; Next 16 builda com turbopack por padrão, webpack fica de fallback). Sentry continua compondo o webpack por cima.
- Code-split (dynamic(import(), { ssr:false })): monaco (CodeEditor) religado em dashboard/files; EditorEmptyState extraído p/ módulo próprio (editor-empty-state.tsx) p/ não arrastar monaco no fallback. recharts virou 4 componentes co-localizados (DashboardActivityChart, SignupsSparkline, ABTestComparisonChart, AnalyticsCharts c/ 6 charts) — recharts saiu do chunk principal das 4 páginas (dashboard, admin/metrics, ab-tests/[id], analytics). xterm/diff2html já eram dynamic (Sub-projeto C) — confirmado.
- framer-motion: cortado onde era barato e de alto valor (público/SEO) — AnimatedSection (era whileInView) e AnimatedCounter (era useInView) reescritos com hook próprio src/hooks/use-in-view.ts (IntersectionObserver) + CSS; API idêntica, 9 consumidores públicos intocados. Remove framer-motion de 8 das 9 páginas públicas. DEFERIDO p/ Sessão 5: background-paths (pathLength infinito em 36 SVGs) e glowing-effect (animate()/mouse-follow) na home + 9 componentes de orchestrations/flow-nodes (auth-gated, AnimatePresence/layout) — não-baratos e/ou em superfície não-mensurável local.
- ISR/RSC: superfície de marketing já é RSC-first com ilhas client; únicas páginas (public) 'use client' = contato/early-access/beta (forms) + api-docs (Swagger via CDN) — todas legitimamente interativas. marketplace já tem ISR (Sprint 2). Nada barato a converter.
tsc limpo (só os 7 erros pré-existentes de deps opcionais: bullmq, e2b, @xterm/xterm, @xterm/addon-fit, diff2html). jest não roda local (OneDrive). Gate real = build EasyPanel + E2E em prod. ⚠️ Score Lighthouse não-confiável nesta máquina — ganho de bundle só confirma na prod VPS.
Follow-ups Sprint 3 (Sessão 5): converter background-paths/glowing-effect + flow-nodes p/ remover framer-motion da home/dashboard; mover data-fetching de useEffect+fetch p/ RSC+Suspense onde valer (dashboard inicial).
Sprint 4 — Dívida técnica e hardening 🟢 ✅ COMPLETO — itens 2-5 DONE + pushed main (2026-06-17); item 1 (drop de órfãos) EXECUTADO em prod (Sessão 6, 2026-06-17): migração `20260617120000_drop_orphan_models` aplicada via migrate deploy no host real `sofia_db@2.24.207.200:5435`.
[STATUS] Entregue (sem migração — drop deferido):
- Security headers (next.config headers()): CSP enforce pragmática (unsafe-inline p/ Next sem nonce + allowlist GTM/GA/unpkg/Sentry; frame-ancestors/object-src/base-uri reais), X-Frame-Options SAMEORIGIN, X-Content-Type-Options nosniff, Referrer-Policy, HSTS (sem includeSubDomains — white-label usa custom domains de clientes), Permissions-Policy. images.remotePatterns: de `**` → allowlist (lh3/*.googleusercontent.com do OAuth + polarisia.com.br/*.polarisia.com.br). ⚠️ CSP precisa validação E2E em prod (não testável local).
- Vazamento de erro: helper safeErrorMessage() em src/lib/api-response.ts (genérico em prod, real em dev; NÃO loga — call-site mantém console.error). 31 substituições em 24 rotas (flows, templates, analytics, agents, monitoring, cron, dev-sessions, knowledge/chunks, teams/run fallback) preservando o shape do envelope. MANTIDOS de propósito: TeamRunError (validação amigável), integrations/[id]/test + tts + elevenlabs (erro de provider externo = feature de debug do dono), health (diagnóstico). public/v1 teams-run já estavam corretos.
- Type safety: JWTPayload ganhou orgId?: string opcional + middleware.ts trocou 4 `(user as any).orgId || user.id` por `user.orgId || user.id` (o cast mascarava campo inexistente — bug de tipo latente). Demais `as any` (groq.ts 23 = Prisma-Json/SDK de LLM; flows-Json) deixados conscientemente: tipá-los arrisca o motor de IA em prod sem teste local. NOTA: "mcp/threads-api ~33 ocorrências" do plano original NÃO existe mais (sprints anteriores limparam; maior cluster atual é groq.ts).
- Pre-commit (husky v9): .husky/pre-commit roda scripts/typecheck.mjs (BLOQUEANTE — tsc --noEmit tolerando SÓ os imports de deps opcionais bullmq/e2b/@xterm/diff2html que faltam local mas existem em CI) + lint advisory dos arquivos staged (NÃO bloqueia — repo tem ~481 erros legados de no-explicit-any; vira gate quando a dívida zerar). prepare: "husky || true" (não quebra npm ci do Docker). scripts: typecheck, db:generate.
Item 1 (modelos órfãos) — ✅ EXECUTADO (Sessão 6, 2026-06-17). Diagnóstico: 7 candidatos verificados um-a-um → 4 órfãos REAIS (Campaign/campaigns [relação User.campaigns; legado→ThreadsCampaign], WhatsappInstance/whatsapp_instances [legado; Evolution API+AgentChannel substituíram], Workspace/workspaces [legado→Organization], ComplianceLog/compliance_logs [UI LGPD existe mas rotas /api/settings/compliance/* NUNCA criadas]) + 3 FALSOS-POSITIVOS eliminados (DocumentEmbedding=raw SQL pgvector, ABTestInteraction=include em ab-tests/[id], ThreadsCampaignPost=include em threads/campaigns). Decisão do usuário: ComplianceLog tratado como dívida morta (dropar junto, não preservar). Drop exige precheck de contagem + olhar dados em prod (lição 6g) + backup JSON + migração + migrate deploy manual no host real ANTES do push. EXECUÇÃO: revalidação confirmou zero accessor/raw-SQL; DESCOBERTA — o plano errava ao dizer que Workspace "não tem relação": `User.workspaceId` é FK real p/ `workspaces` (não usada no código → coluna dropada junto, decisão do usuário). Precheck no host real `sofia_db@2.24.207.200:5435`: as 4 tabelas VAZIAS (0 linhas) + 0 users com workspace_id (zero dado de cliente). Backup em `docs/superpowers/backups/2026-06-17-orphan-models-pre-drop.json`. Migração `20260617120000_drop_orphan_models` (DROP das 4 tabelas + ALTER users DROP COLUMN workspace_id) aplicada via migrate deploy ANTES do push; verificado: tabelas sumiram, users(21)/teams/team_runs/scheduled_team_runs intactas. UI órfã removida (page `dashboard/settings/compliance` + card no menu settings + import Scale). tsc limpo.
Limpeza "Orchestration"→"Teams": copy/i18n/eventos (first_orchestration_*), chave interna orchestrationsExecuted (follow-up deferido do programa SP6) — sem find-replace cego (SEO-sensível). NÃO feito (fora do escopo desta sessão).
tsc limpo (só os 7 erros pré-existentes de deps opcionais). jest não roda local (OneDrive). E2E autenticado em prod pendente (gate real). Próximo: Sessão 6 (drop órfãos + Sprint 5 Testes).
Sprint 5 — Testes 🟢 ✅ DONE (Sessão 7, 2026-06-17) — FECHA O ROADMAP DE REFATORAÇÃO. Sem migração, não toca o DB de prod.
[STATUS] Entregue:
- CI gate de testes LIGADO (o achado crítico era que jest não rodava em lugar nenhum: nem local — OneDrive errno -4094 — nem no CI, que só fazia typecheck+lint+build → escrever teste era teatro). Novo step "Test (jest)" em .github/workflows/ci.yml DEPOIS do typecheck, com as mesmas env vars dummy. Roda `npm test -- --ci` SEM --coverage de propósito: o gate falha por TESTE quebrado (regressão), não por coverage — o threshold global de 50% ainda não foi atingido (cobertura de rota subindo) e travaria o PR cedo demais. Quando passar de 50%, trocar por `npm run test:coverage -- --ci`.
- 8 arquivos de teste de rota novos (src/__tests__/integration/), priorizando a regressão de IDOR/ownership do Sprint 0 (maior risco, mais fácil de regredir): teams.test.ts (listagem/POST escopo dono + [id] GET/PATCH/DELETE), flows-ownership.test.ts, conversations-ownership.test.ts, ab-tests-ownership.test.ts, integrations-ownership.test.ts, skills-ownership.test.ts, auth-login.test.ts (429 rate-limit/400 zod/401 cred/200), auth-register.test.ts (400 zod senha curta/409 email em uso/201 cria user+trial Pro). Padrão copiado de agents-ownership.test.ts: mockar getAuthFromRequest + prisma (só os models/métodos usados); asserts no `where` (createdBy/userId presente p/ dono, undefined p/ admin quando há bypass) + status. parseJson/zod schemas rodam de verdade.
- DESCOBERTA (consagrada nos testes): NEM TODA rota dá bypass de admin. Só as que usam `ownerId(auth)` no where (agents, ab-tests, integrations[campo userId]). teams/[id], teams(listagem), flows/[id], conversations/[id](via agent.createdBy) e skills/[id] escopam por `auth.id` DIRETO → admin só vê os próprios recursos. skills/[id] tem regra extra: GET vê builtin OU dono; PUT/DELETE só dono e não-builtin (builtins protegidas). Os testes documentam o comportamento REAL de cada uma (não alteram lógica).
Verificação: `npm run typecheck` limpo (só os 7 erros pré-existentes de deps opcionais) — confirma que os 8 .test.ts compilam. jest NÃO roda local (OneDrive errno -4094, confirmado ao vivo nesta sessão) → gate real = CI (agora ligado). E2E autenticado em prod pendente (gate de produto, com o usuário).
🏁 ROADMAP DE REFATORAÇÃO (Sprints 0-5) COMPLETO.
