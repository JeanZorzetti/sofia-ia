  ...
  try {
    for (let turn = 0; turn < config.maxTurns; turn++) {
      ...
      try {
        ...
        ], undefined, { model: lead.model, effort: lead.effort, capabilities: lead.capabilities, memberId: lead.id, runId })
      } catch (e) {
      ...
      if (runnable.length > 0) {
        ...
        const outcomes = await runWithConcurrency(runnable, maxParallel, async (t): Promise<TurnOutcome> => {
          ...
          try {
            ...
            out = await chat(worker.agentId, [{ role: 'user', content: buildTaskPrompt(t, t.reviewNote, { allowClarify: true }) }], undefined, { model: worker.model, effort: worker.effort, taskId: t.id, runId, capabilities: worker.capabilities, memberId: worker.id })
          } catch (e) {
      ...
      if (reviewer) {
        ...
        if (toReview.length > 0) {
          ...
          const outcomes = await runWithConcurrency(toReview, maxParallel, async (t): Promise<TurnOutcome> => {
            ...
            let out: ChatResult
            try {
              out = await chat(reviewer.agentId, [{ role: 'user', content: buildReviewPrompt(t, diff) }], undefined, { model: reviewer.model, effort: reviewer.effort, capabilities: reviewer.capabilities, memberId: reviewer.id, runId })
            } catch (e) {
              if (isRateLimit(e)) return { kind: 'rate_limited' }
              return { kind: 'error', error: e as Error }
            }
      ...
      if (isBoardSettled(board)) {
        ...
        try {
          conso = await chat(lead.agentId, [{ role: 'user', content: buildConsolidationPrompt(board) }], undefined, { model: lead.model, effort: lead.effort, capabilities: lead.capabilities, memberId: lead.id, runId })
        } catch (e) {
