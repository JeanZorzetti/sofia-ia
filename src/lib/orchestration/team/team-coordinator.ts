    ...
    for (let turn = 0; turn < config.maxTurns; turn++) {
      ...
      for (const t of todo) {
        ...
        try {
          ...
          out = await chat(worker.agentId, [{ role: 'user', content: buildTaskPrompt(t, t.reviewNote) }], undefined, { model: worker.model, effort: worker.effort, taskId: t.id, runId, capabilities: worker.capabilities, memberId: worker.id })
        } catch (e) {
