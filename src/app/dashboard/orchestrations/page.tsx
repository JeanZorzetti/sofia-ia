...
export default function OrchestrationsPage() {
  ...
  const [aiCreating, setAiCreating] = useState<boolean>(false)
  ...
  const handleCreateFromAi = async () => {
    ...
    try {
      ...
      for (const agentDef of aiSuggestion.agents) {
        const res = await fetch('/api/agents', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: agentDef.role,
            description: `Agente criado automaticamente para a orquestração "${aiSuggestion.name}"`,
      ...
      const agentSteps = aiSuggestion.agents.map((a, i) => ({
      ...
      const orchRes = await fetch('/api/orchestrations', {
        ...
        body: JSON.stringify({
          ...
          strategy: aiSuggestion.strategy || 'sequential',
          agents: agentSteps,
        }),
      })
      ...
      fetchOrchestrations()
      fetchAgents()
      router.push(`/dashboard/orchestrations/${orchData.data.id}`)
    } catch (err: unknown) {
    ...
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            onClick={() => { setAiSuggestion(null); setAiDescription(''); setAiDialogOpen(true) }}
      ...
      <Dialog open={aiDialogOpen} onOpenChange={setAiDialogOpen}>
        <DialogContent className="bg-gray-900 border-white/20 text-white max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-white/60">
              Descreva o processo que você quer automatizar. A IA vai sugerir os agentes, prompts e estrutura.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            ...
            {aiSuggestion && (
              <div className="space-y-4 border border-purple-500/30 rounded-xl p-4 bg-purple-500/5">
                ...
                {/* Agents preview */}
                <div>
                  <p className="text-xs text-white/40 mb-2">Agentes sugeridos ({aiSuggestion.agents.length}):</p>
                ...
                {aiSuggestion.suggestedInput && (
                  <div className="p-3 rounded-lg bg-green-500/10 border border-green-500/20">
                    ...
                    <p className="text-xs text-white/60 italic">&ldquo;{aiSuggestion.suggestedInput}&rdquo;</p>
                  </div>
                )}
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  >
                    {aiCreating ? (
                      <><Loader2 className="h-4 w-4 animate-spin" /> Criando agentes e orquestração...</>
                    ) : (
